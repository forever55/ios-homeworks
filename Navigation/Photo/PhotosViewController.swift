...
//  Navigation
//
//  Created by Александр on 17.05.2022.
//
...
final class PhotosViewController: UIViewController {
    ...
    let detailPhotoView = PhotosCollectionView ()
...
extension PhotosViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let photos = imageNameList[indexPath.row]
        cell.photoGalleryImages.image = UIImage(named: photos.imageNameList)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        UIView.animate(withDuration: 0.5) { [self] in
            detailPhotoView.photoView.image = UIImage(named: imageNameList[indexPath.row].imageNameList)
